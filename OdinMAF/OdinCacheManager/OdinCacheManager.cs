...
namespace OdinPlugs.OdinMAF.OdinCacheManager
{
    public class OdinCacheManager : IOdinCacheManager
    {
        ...
        public OdinCacheManager(ConfigOptions options)
        {

            if (cacheManager == null)
            {
                var sysRuntimeCacheOpt = options.CacheManager.Handles.Where(s => s.KnownType == "SystemRuntime").Single();
                var redisCacheOpt = options.CacheManager.Handles.Where(s => s.KnownType == "Redis").Single();
                var redisOpt = options.Redis;
                cacheManager = CacheFactory.Build(options.CacheManager.CacheName, settings =>
                {
                    settings.WithUpdateMode(options.CacheManager.UpdateMode.ToEnum<CacheUpdateMode>())
                        ...
                        .WithRedisConfiguration(redisCacheOpt.HandleName, config => //Redis缓存配置
                        {
                            config.WithAllowAdmin()
                                .WithPassword(redisOpt.RedisPassword)
                                .WithDatabase(redisOpt.DefaultDatabase)
                                .WithEndpoint(redisOpt.RedisIp, redisOpt.RedisPort);
                        })
                        .WithJsonSerializer()
                        .WithMaxRetries(options.CacheManager.MaxRetries) //尝试次数
                        .WithRetryTimeout(options.CacheManager.RetryTimeout) //尝试超时时间
                        .WithRedisBackplane(options.CacheManager.BackPlane.KnownType)
