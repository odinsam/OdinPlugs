...
namespace OdinPlugs.OdinMAF.OdinHystrix
{
    ...
    public class OdinHystrixAttribute : AbstractInterceptorAttribute
    {
        /// <summary>
        /// 最多重试几次，如果为0则不重试
        /// </summary>
        ...
        public OdinHystrixAttribute(string fallBackMethod)
        {
        ...
        public override async Task Invoke(AspectContext context, AspectDelegate next)
        {
            ...
            lock (policies)//因为Invoke可能是并发调用，因此要确保policies赋值的线程安全
            {
                if (policy == null)
                {
                    System.Console.WriteLine($"policy ---- null");
                    ...
                    if (MaxRetryCount > 0)
                    {
                        policy = policy.WrapAsync(Policy.Handle<Exception>().WaitAndRetryAsync(MaxRetryCount, i => TimeSpan.FromMilliseconds(RetryIntervalMilliseconds)));
                    }
